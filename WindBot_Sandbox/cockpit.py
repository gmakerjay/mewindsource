...
def run_live_duel_loop(deck, opponent, opp_deck, iterations, progress_log):
    ...
    for i in range(1, iterations + 1):
        ...
        try:
            ...
            time.sleep(1.0)
            p2 = subprocess.Popen(
                [windbot_exe, f"name={opponent}", f"deck={opp_deck}", "port=7911", "hostinfo=", "version=720937"],
                cwd=windbot_dir,
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT
            )
...
HTML_TEMPLATE = _load_template("dashboard.html")
...
class CockpitHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if url.path == '/api/train':
            ...
            success = self.start_training(deck, opponent, mode, iterations)
    ...
    def start_training(self, deck, opponent, mode, iterations):
        global active_process, active_bots
        kill_active_process()
        ...
        if mode == 'heuristic':
            ...
            try:
                threading.Thread(
                    ...
                    args=(deck, opponent, opp_deck, iterations, progress_log),
                    daemon=True
                ).start()
                return True
            except Exception as e:
