# Handoff Report: Dreadnought Archetype AI Design

## 1. Observation
- **Deck Configuration**: We verified `deck/2026_Dreadnought.ydk` which contains the following cards:
  - Custom Cards:
    - `101402022`: Destiny HERO - Doom Liege (เดวิลโหลดกอย - Combo Starter)
    - `101402062`: Clock Tower Prison City - Dark City (Field Spell - search/SS on destroy)
    - `101402023`: Destiny HERO - Dreadnought Servant (Extender - summon/destroy to search Poly/GY spin)
    - `101402037`: Destiny HERO - Dreadnought (Boss - Fusion beater/Draw +2)
    - `101402021`: Destiny HERO - Death Dogma (Boss - Summon by GY banish/Burn 2000/Quick Fusion)
    - `100456010`: D - Burst (Supporting Spell)
  - Other Main Support Cards:
    - `10808715`: Masked HERO Dusk Crow
    - `58288218`: Masked HERO Furnace
    - `66206748`: Masked HERO Fountain
    - `83965311`: Destiny HERO - Plasma
...
- **Existing Framework**:
  - `BaseCustomExecutor.cs` implements the base class `BaseCustomExecutor` (inheriting from `DefaultExecutor`), defining LP monitoring, logging, and default execution fallbacks.
  - `UnifiedIgnisExecutor.cs` registers subclasses by deck name.
  - `InvokeExecutor.cs` demonstrates subclassing `UnifiedIgnisExecutor`, clearing default dynamic weight-based executors, and explicitly registering card delegates via `AddExecutor(ExecutorType, cardId, delegate)`.
- **Iron Rules (`Docs/Rules.md`)**:
  - Rule #1: Block handtraps on own turn (lines 1681–1690).
  - Rule #2: Block self-hurt chaining (lines 1670–1679).
  - Rule #3: Target availability check (lines 1711–1719 for Called by Grave, 1731–1739 for Impermanence).
  - Rule #4: Catch-all fallback must return `false` (BaseRules).
  - Rule #5: Priority Hard Cap.
  - Rule #6: OnChaining check direction (our card controller == 0).
  - Rule #7: GetNextPlan loop back.
  - Rule #8: `_learningApplied` guard.

---

## 2. Logic Chain
1. **Clear Dynamic Executors**: To enforce strict, prioritized sequencing, `DreadnoughtExecutor` must call `Executors.Clear()` in its constructor, mirroring `InvokeExecutor`.
2. **Explicit Priority-based Registrations**:
   - High-priority interruptions/handtraps (Ash, Impermanence, Called by Grave, Fuwalos, Dominus Spark) are registered first with conditions that check opponent controller/turn (Rule #1).
   - Searchers/Fields (Terraforming, Dark City, Doom Liege) follow to establish start-of-turn setups.
   - Extenders and Special Summons (Dreadnought Servant, Dusk Crow, Furnace, Fountain, Plasma, Death Dogma) follow next to fill the board.
   - Normal Summons (Vyon, Doom Liege, Servant, Shadow Mist) are prioritized if special summon plays are completed or unavailable.
   - Spells & Fusion engines (Fusion Destiny, Polymerization, Mask Change, D-Burst) follow.
   - Extra Deck/Boss triggers (Dreadnought, DPE, Dystopia, Contrast HERO Chaos, Denier/Malicious GY triggers) are resolved last to maximize payoff.
3. **Helper Methods**:
   - `IsDestinyHero(card)` and `IsHero(card)` check ID sets directly, bypassing potential wrappers/CDB failures.
   - `GetGraveWarriorDarkCount()` and `GetGraveHeroCount()` compute counts for Death Dogma and Dusk Crow requirements.
4. **Target Select Optimizations (`OnSelectCard`)**:
   - Override `OnSelectCard` to:
     - Target `Clock Tower Prison City - Dark City` for destruction by `Dreadnought Servant` or `D - Burst`.
     - Target the strongest opponent monster for `Doom Liege` banishing or `Plasma` absorption.
     - Select follow-up pieces (Death Dogma, Doom Liege) when searching with `Dreadnought` or `Dark City`.
     - Recycle `Malicious` or `Denier` when executing `Denier` or `Death Dogma` (Quick Fusion) to maximize resource loops.

---

## 3. Caveats
- Since this is a read-only analysis role, the draft code is presented in this file rather than written to `WindBot/DreadnoughtExecutor.cs` directly.
- The C# compiler version is assumed to support standard C# features utilized in `InvokeExecutor.cs` (e.g. C# 7.3+).

---

## 4. Conclusion
We designed the C# executor class `DreadnoughtExecutor.cs` in compliance with all codebase patterns and the 8 Iron Rules. The proposed file content is detailed below.

### Proposed Code: `DreadnoughtExecutor.cs`
...
namespace ProjectIgnisAI
{
    ...
    public class DreadnoughtExecutor : UnifiedIgnisExecutor
    {
        ...
        public DreadnoughtExecutor(GameAI ai, Duel duel) : base(ai, duel)
        {
            ...
            AddExecutor(ExecutorType.Activate, 24224831, CalledByGraveEffect); // Called by the Grave (ID 2)
            ...
            AddExecutor(ExecutorType.Activate, 40237840, SabatielEffect); // Sabatiel (Search Poly/Fusion)
            ...
            AddExecutor(ExecutorType.SpSummon, 83965311, PlasmaSpSummon); // Plasma SS
            AddExecutor(ExecutorType.Activate, 83965311, PlasmaEffect); // Plasma Absorb
            ...
            AddExecutor(ExecutorType.Activate, 21143941, MaskChangeEffect); // Mask Change
        ...
        private bool IsHero(ClientCard card)
        {
            ...
            return id == 10808715  // Dusk Crow
                ...
                || id == 46759931  // Trinity
                || id == 58481573  // Dark Law
                || id == 63813056  // Blast
                || id == 1948619   // Wonder Driver
                || id == 58004362  // Cross Crusader
                || id == 87758526  // Shining Flare Wingman
                || id == 13243125  // Flame Wingman
                || id == 54757758; // Acid
        }
        ...
        private bool MaskedHeroFurnaceEffect()
        {
            if (Card.Location == CardLocation.Hand)
            {
                return (!HasInHand(21143941) && !HasInHand(24094653)) && Bot.Hand.Count > 1;
            }
            return false;
        }
        ...
        {
            return Card.Location == CardLocation.Hand || Card.Location == CardLocation.Graveyard;
        }
        ...
        {
            if (Card.Location != CardLocation.Hand && Card.Location != CardLocation.Graveyard) return false;
        ...
        {
            return Card.Location == CardLocation.MonsterZone || Card.Location == CardLocation.Graveyard;
        }
            ...
            else if (Duel.Phase == DuelPhase.Main1 || Duel.Phase == DuelPhase.Main2)
            {
                foreach (var m in Bot.GetMonsters())
                {
                    if (m != null && m.IsFaceup() && IsHero(m) && m.Attribute == (int)CardAttribute.Dark)
                        return !HasInMonsterZone(58481573);
                }
            }
            return false;
        }
        ...
        {
            return Card.Location == CardLocation.Graveyard;
        }
        ...
        protected override bool EvaluateCardAction(ClientCard card, CardMetadata meta, ExecutorType type)
        {
            ...
            {
                if (card.Id == 101402021 || card.Id == 83965311 || card.Id == 40591390 || card.Id == 17132130)
                {
        ...
        public override IList<ClientCard> OnSelectCard(IList<ClientCard> cards, int min, int max, long hint, bool cancelable)
        {
            ...
            if (Card != null && (Card.Id == 101402022 || Card.Id == 83965311) && hint == 0)
            {
            ...
            if (hint == HintMsg_AddToHand)
            {
                ...
                if (Card != null && Card.Id == 58288218)
                {
                    foreach (var c in cards)
                    {
                        if (c.Id == 21143941) return new List<ClientCard> { c }; // Mask Change
...
1. **Compilation**: Run the project's build command inside the `Developer/BrainStorms/windbot-master` folder:
   ```powershell
   dotnet build Developer/BrainStorms/windbot-master/WindBot.csproj
   ```
2. **Subclass Registration Check**: Verify that `DreadnoughtExecutor` registers itself via the `[Deck("2026_Dreadnought", "2026_Dreadnought")]` attribute, allowing the engine's `LoadConfiguration` and factory method to load it dynamically.
3. **Iron Rule Verification**:
   - Trace the handtrap evaluation methods (e.g. `AshBlossomEffect`, `ImpermanenceEffect`) to verify they return `false` on player turn `0` (conforming to Rule #1).
   - Trace the chaining logic to verify it does not chain to player `0` card activations unless specified (conforming to Rule #2).
